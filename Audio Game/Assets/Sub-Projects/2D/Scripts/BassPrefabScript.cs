...
public class BassPrefabScript : MonoBehaviour, WallInterface {
	...
	void Update() {
		Scale();
		if (SpawnManager.activeB.Count > 0)
			if (SpawnManager.activeB[0] == this.gameObject) Tick();
	...
	void RemoveFromActiveListIfPastAllTriggers() {
		...
		if (!removed && transform.position.z <= edgeOfWallLeavesTrigger) {
			...
			SpawnManager.activeB.RemoveAt(0);
			removed = true;
		}
	}
	...
	void Tick() {
		...
		if (firstPress && Input.GetKeyDown(KeyCode.LeftArrow)) {
			keyPressed = true;
			firstPress = false;
			tick = 0;
		}
	...
	void OnTriggerEnter(Collider col) {
		...
		if (col.tag == "Trigger") {
			if (keyPressed && !triggered) {
				...
				triggered = true;
				int score = Mathf.FloorToInt(AudioPlayer.freqBands[1] * 100 * ScoreManager.multiplier);
	...
	void BackToPool() {
		...
		SpawnManager.bassWallPool.Add(this.gameObject);
	}
}
